...
Clarinet.test({
  ...
  async fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    const xyzToken = new XyzToken(chain, deployer);
    let result = xyzToken.balanceOf(txSender.address).result;
    ...
    const [deposit2, redeem2] = block.receipts;
    deposit2.result.expectOk();
    redeem2.result.expectOk();

    // Check balance in the assignee address.
    result = xyzToken.balanceOf(txSender.address).result;
    result.expectOk().expectUint(250);
  }
});
...
Clarinet.test({
  ...
  async fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    notRedeemed.result.expectOk().expectBool(true);
